...
仓库采集 `.zshrc`、`.bashrc`、`.bash_profile` 和 `.bash_logout`。`state/system-info.txt` 记录的当前登录 Shell 是 Fish，但 Fish 用户配置目前不在采集白名单中。Git 配置保存为 `.gitconfig`，公开快照中的 `user.email` 会由 `capture.sh` 删除。
