...
Shell startup files are captured from the home directory: `.zshrc`, `.bashrc`, `.bash_profile`, and `.bash_logout`. The current login shell recorded in `state/system-info.txt` is Fish; Fish itself currently has no user path in the capture allowlist. Git configuration is captured as `.gitconfig`, but `capture.sh` removes `user.email` from the public copy.
