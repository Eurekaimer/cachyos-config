...
| Images render but "download failed" keeps popping up | The Markdown integration re-requests every visible remote image on each render pass, so an intermittent proxy TLS drop (curl 35/56) fails the same URL repeatedly. This config lets curl retry with `--retry-all-errors` and reports each URL at most once per session. |
